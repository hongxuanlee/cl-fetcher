...
var addCookie = function addCookie(cookieStr, url) {
	...
	cookieArr.forEach(function (item) {
		_log2.default.debug(JSON.stringify(item));
...
var saveTree = function saveTree(tree, name, options) {
	options = options || {};
	var root = options.root ? options.root : phantom.libraryPath + '/../../data';
	var dirname = options.dirname ? options.dirname : dirName;
	if (!name) {
		var time = Date.now();
		name = time;
	}
	...
	if (_fs2.default.exists(dir) || _fs2.default.makeDirectory(dir)) {
		_log2.default.debug('save json in  [' + dir + ']');
		_fs2.default.write(dir + '/' + name + '.json', tree);
		return true;
	} else {
...
var ergodicState = function ergodicState(curpage, pageContent, isRecover, remain) {
	if (!isRecover) {
		...
		_log2.default.debug('treeJson:', JSON.stringify(treeJson));
	...
	var evaluateRet = evaluate(curpage, _evaluate2.default, {}, remain);
	...
	if (statusChange) {
		...
		if (pageStateMatrix.length > 0) {
			var curStatus = pageStateMatrix.pop();
